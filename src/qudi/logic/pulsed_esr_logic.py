...
class PulsedESRLogic(LogicBase):
    ...
    _pulse_blaster_hardware = Connector(name='pulse_blaster_hardware',
                                   interface='PulseBlasterHardware',
                                   optional=True)
    ...
    def Send_to_Pulse_Blaster(self,Flat_exp):
        ...
        start=spinapi.pb_inst_pbonly(int(sum(Flat_exp[0].channel_binary[0])),spinapi.Inst.LOOP,1,(Flat_exp[0].end_tail-Flat_exp[0].start_tail)*spinapi.us) # generates a loop of instruction here only one iteration
        print(f"Flat_exp[0].channel_binary:{Flat_exp[0].channel_binary[0]}")
        print(f"spinapi.pb_inst_pbonly({sum(Flat_exp[0].channel_binary[0])},spinapi.Inst.LOOP,{1},({Flat_exp[0].end_tail-Flat_exp[0].start_tail})*spinapi.us)") 
        for i in range(1,len(Flat_exp)):# we start from one because we already did the 0 index
            if i!=len(Flat_exp) -1:
                ...
                spinapi.pb_inst_pbonly(int(sum(Flat_exp[i].channel_binary[0])),spinapi.Inst.CONTINUE,0,(Flat_exp[i].end_tail-Flat_exp[i].start_tail)*spinapi.us)
            else:
                print(f"spinapi.pb_inst_pbonly({sum(list(Flat_exp[i].channel_binary[0]))},Inst.CONTINUE,0,({Flat_exp[i].end_tail-Flat_exp[i].start_tail})*spinapi.us)")
                spinapi.pb_inst_pbonly(int(sum(Flat_exp[i].channel_binary[0])),spinapi.Inst.CONTINUE,0,(Flat_exp[i].end_tail-Flat_exp[i].start_tail)*spinapi.us)
                ...
                spinapi.pb_inst_pbonly(int(sum(Flat_exp[i].channel_binary[0])),spinapi.Inst.END_LOOP,start,Flat_exp[i].end_tail-Flat_exp[i].start_tail)
        spinapi.pb_inst_pbonly(int(0),spinapi.Inst.STOP,0,1*spinapi.us) # This instruction stops the pulse sequence. The duration is set to a very small value to ensure the stop instruction is executed almost immediately.
    ...
    def create_counter_task(self):
        ...
        counter_task.timing.cfg_samp_clk_timing(
            ...
            sample_mode=nidaqmx.AcquisitionType.CONTINUOUS,
            samps_per_chan=100000)
        #una continuiacion de lo de arriba
        counter_task.triggers.pause_trigger.dig_lvl_src=self.gate_pin
        counter_task.triggers.pause_trigger.dig_lvl_when=nidaqmx.constants.Level.LOW # que pause de contar cuando este en low
        counter_task.triggers.pause_trigger.trig_type=nidaqmx.constants.TriggerType.DIGITAL_LEVEL
...
class Pulse:
    ...
    def __init__(self,start_tail,end_tail,channel_binary):
